...
function formatForecastCardDate(date) {
...
function mapForecastStatus(label) {
...
export default function Dashboard() {
  ...
  const forecastThreshold = Number.isFinite(Number(forecastData?.threshold))
    ...
    : "-";
  const forecastStartDate = forecastData?.forecast_date
    ? new Date(forecastData.forecast_date)
    : new Date();
  const forecastStatus = mapForecastStatus(forecastData?.prediction_label);
  const forecastCards = [0, 1, 2].map((offset) => {
    const cardDate = new Date(forecastStartDate);
    cardDate.setDate(forecastStartDate.getDate() + offset);
    return {
      dateStr: formatForecastCardDate(cardDate),
      status: forecastStatus.status,
      probability: forecastChancePercent,
      color: forecastStatus.color,
      bg: forecastStatus.bg,
      icon: forecastStatus.icon,
    };
  });
              ...
              <div className="mt-auto pt-6 pb-2">
                ...
                {forecastLoading && (
                  <div className="grid grid-cols-3 gap-3">
                    {[1, 2, 3].map((idx) => (
                      <div
                        key={`forecast-skeleton-${idx}`}
                        className="relative rounded-xl p-3 flex flex-col items-center text-center border border-transparent bg-gray-50/60 animate-pulse"
                      >
                ...
                {!forecastLoading && !forecastError && forecastData && (
                  <>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {forecastCards.map((item, idx) => (
                        ...
                        >
                          ...
                          <div className="font-extrabold text-sm uppercase" style={{ color: item.color }}>
                            {item.status}
                          </div>
                    ...
                    <div className="text-[11px] text-gray-500 flex flex-wrap gap-4 mt-3">
